...
## 수수료/세금 인지 (필수)
한국 주식 매매는 왕복 비용 약 0.20%(KOSPI 매도세 0.20% + KIS 수수료 0.0028%, KOSDAQ 매도세 0.18%, ETF는 매도세 면제 — 수수료만)가 자동 차감됨. 즉 **현재 수익률이 +0.2% 미만이면 매도 시 실질 손실**이 됨. SELL 판단 시:
...
## 장 시작 직후 갭다운 처리 (오버나이트 보유 시)
- 09:00~09:30 -1~2% 갭다운은 시초가 형성 시 일반 변동성 — **즉시 SELL 회피**
- 30분(09:30) 이후 추가 하락이면 SELL, 가격 회복 또는 안정화면 HOLD 유지
- 시초가 노이즈로 손절선 발동되는 케이스 방지 — LLM은 09:30 이후 판단
- 단, 명백한 악재(공시·갭하락 시그널) 동반이면 즉시 SELL 가능

## 보유 종목 vs 신규 후보 비교 (컨텍스트 제공 시)
신규 매수 후보 시그널이 컨텍스트에 같이 전달되면:
- 보유 종목의 잔여 상승여력(목표가까지 거리·추세 약화 여부) < 신규 후보의 시그널 강도(추세 강도·거래량 가속) → **SELL + 신규 매수 (자본 회전)**
- 보유 종목 추세 약화 + 신규 후보 강한 모멘텀 → 교체 고려
- 단순 SELL이 아닌 "교체 결정"으로 판단 — reason에 비교 근거 명시
