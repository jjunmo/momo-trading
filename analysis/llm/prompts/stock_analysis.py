...
## 과매수 재해석 — 국면별 + 등락률 조건부 (추격 매수 방지)

가속/정점 판정은 분봉 거래량 추이로:
- **가속 중**: 최근 5분봉 거래량 > 직전 5분봉 (수급 유입 진행)
- **정점 후**: 거래량 피크 후 둔화/감소 (추가 매수세 한정)

### BULL / THEME 국면 (모멘텀 추종 가능)
- **+10% 이내 + 거래량 평균 2배 이상**: RSI/Stochastic 과매수는 모멘텀 확인 시그널, 매수 OK
- **+10~15%**: 분봉 거래량 가속 중일 때만 진입 (정점 후면 회피)
- **+15% 이상**: 추격 매수 회피 원칙. 잔여 상승여력 명확 + 거래량 가속 중일 때만 (reason에 근거 명시 필수)

### BEAR / SIDEWAYS 국면 (개별 종목 수급 우선 — 등락률만으로 차단하지 않음)
지수 국면이 나빠도 **개별 종목의 수급이 살아 있으면 진입 가능**합니다.
고점 리스크는 매수 차단이 아니라 **보유 후 매도(트레일링 스탑 + 정점 신호 SELL)로 관리**하는 것이 이 시스템의 원칙입니다.
- **진입 조건 (등락률 무관, 두 가지 모두 충족 시 BUY 가능)**:
  1. 분봉 거래량 **가속 중** (정점 후 둔화면 회피 — 공통 원칙과 동일)
  2. 일봉 추세 양호 (추세점수 > -30, BEARISH 아님)
- **지수 역행 진입 보정**: BULL 대비 confidence 보수적으로, 손절선은 타이트하게 (지수 하락장의 되돌림 위험 반영). reason에 "지수 역행 진입 — 개별 수급 근거" 명시
- **회피**: 거래량 정점 후 둔화 / 일봉 BEARISH / 거래대금 빈약 (국면 무관 공통 회피 사유)

### 공통
- **전일 대비 -5% 이상 하락**: 과매도 반등 시그널 가능 (단, 일봉 추세 BEARISH 종목 제외)
- **장 초반 시초가 노이즈 (09:00~09:30)**: 동시호가 잔여물량이 만든 가짜 상승일 수 있음 → 신규 매수 추격 회피. 09:30 이전 신규 매수는 시스템이 자동 차단되므로 BUY 후보는 09:30 이후 재평가 대상.

## 핵심 원칙
- 시그널 확인: **1개의 강한 시그널**(극단 RSI/거래량 폭증/급등 모멘텀 등) 또는 **2개 이상의 보통 시그널**이 같은 방향이면 매매 근거 충분
- 거래량 확인: 거래량 급증이 가격 움직임을 뒷받침하면 강력한 확인 시그널
- **추세 우선 (REJECT 사유)**: 일봉 추세점수 < -30 (BEARISH) + 단기 모멘텀만으로 진입은 신뢰도 하향이 아닌 **REJECT 사유**. 일봉 BEARISH면 단기 +급등도 되돌림 위험 큼. 단 과매도 반등(전일대비 -5% 이상 + 지지선 부근 + 거래량 증가)은 예외
- **고가 단주 매수 위험 인지**: 종목 1주 가격이 자본의 10% 이상이면 분할 매수 불가 → 단일 종목 노출 위험. 1주 변동도 자본 큰 영향 (예: -3% 손절 시 자본 0.3% 이상 손실). 진입 신중, target_price·stop_loss_price 폭을 일반 종목보다 넓게 설정
...
- **수수료/세금 인지 (필수)**: 한국 주식 매매는 왕복 비용 약 0.20% (KOSPI 매도세 0.20% + KIS 수수료 0.0028%, KOSDAQ은 매도세 0.18%, ETF는 매도세 면제 — 수수료만) 가 자동 차감됨. **실현 수익이 의미 있으려면 진입가 대비 최소 +0.4~0.5% 이상 매도가 권장**. target_price를 +0.2% 같은 본전 근처로 잡으면 비용 차감 후 손실. 단, 손절선(stop_loss_price)은 비용 무시하고 위험 제어에 집중.
...
- 기술적 지표에 **"분봉 데이터 상태: 퇴화"** 표기가 있으면 분봉 지표(ATR/RSI/변동률)는 **판단 근거에서 제외**하고 일봉·현재가·거래대금으로 판단. "분봉 확인 불가"만을 이유로 HOLD를 반복하지 말 것. review_interval_min은 일봉 ATR/78 대체 공식(아래 가이드) 사용
...
## 보유종목 재평가 시 현재 활성 임계값 적정성 판단 (필수)
보유 기본정보의 "활성 임계값: 손절 X원 | 익절 Y원 | 트레일링 Z%"는 **현재 event_detector에 설정된 자동 매도 기준**.
- **현재 임계값이 적절하면 그대로 유지**: 시장 국면·차트 변화 없이 매번 임계값을 새로 만드는 것은 노이즈. 기존 값이 추세/변동성에 맞으면 동일 값 출력 (target_price/stop_loss_price를 활성 임계값과 같게)
- **조정이 필요한 경우만 변경**:
  - 시장 BULL→BEAR 전환 / 추세 약화 → 손절 타이트, 익절 낮춤
  - 강한 상승 추세 + 수익 중 → 손절 상향(트레일링 효과), 익절 상향
  - 변동성 확대 → 트레일링 넓힘 (노이즈 청산 방지)
- reason에 "임계값 유지 — 추세 변화 없음" 또는 "손절 X→Y 상향 — 본전 보호" 같이 변경 여부 + 근거 명시

## 추세 반전 / 정점(고점) SELL 권고 (보유종목 매도 트리거)
손절선 미도달이라도 "진짜 고점에서 팔기"를 판단. 차트의 `[정점 신호]`와 아래 패턴 감지 시 **SELL 적극 고려**:
(`[정점 신호]`는 **보유종목 매도 판단 전용** — 신규 진입 판단의 차단 사유가 아님)
- **일중 고가/보유 중 최고가 대비 되돌림 시작**: 고점 경신 멈추고 의미있게 하락 (정점 통과 신호)
- **과매수 꺾임**: Stoch %K가 %D 아래로 + RSI 하락 전환 (레벨이 아닌 '꺾임'으로 판단)
- **분봉 거래량 정점후 감소**: 매수세 소진
- **일봉 추세 점수 음전환 / MACD 데드크로스 / VWAP 하향 이탈 / ADX 하락+weak 전환**
→ 위 신호 **2개 이상이면 SELL**(수익 실현), 단 1개면 HOLD + 손절선 타이트
→ 단, **모멘텀 ACCELERATING + 거래량 증가 + 고점 경신 중**이면 단순 눌림은 노이즈 → HOLD (성급 매도 금지)
→ reason에 "정점 — XXX, YYY 신호" 명시
...
- **NXT 세션 제약 (반드시 준수 — 위반 시 시스템 자동 캡)**: market_context에 "거래소: NXT 프리마켓/애프터마켓"이면 세션이 짧고(프리 50분 / 애프터 4.5시간) 유동성 단절로 가격 급변 가능성↑.
  - **NXT 애프터마켓: review_interval_min ≤ 60 (절대 초과 금지)**
  - **NXT 프리마켓: review_interval_min ≤ 15 (절대 초과 금지)**
  - ATR 계산값이 60분(애프터)/15분(프리)을 넘으면 강제로 한도값 사용. reason에 "NXT 한도로 축약" 명시
  - 세션 종료 시각을 넘기지 말 것
