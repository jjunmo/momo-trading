...
- **수수료/세금 인지 (필수)**: 한국 주식 매매는 왕복 비용 약 0.20% (KOSPI 매도세 0.20% + KIS 수수료 0.0028%, KOSDAQ은 매도세 0.18%) 가 자동 차감됨. **실현 수익이 의미 있으려면 진입가 대비 최소 +0.4~0.5% 이상 매도가 권장**. target_price를 +0.2% 같은 본전 근처로 잡으면 비용 차감 후 손실. 단, 손절선(stop_loss_price)은 비용 무시하고 위험 제어에 집중.
