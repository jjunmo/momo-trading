...
- **NXT 세션 제약**: market_context에 "거래소: NXT 프리마켓/애프터마켓"이면 세션 자체가 짧고(프리 50분 / 애프터 4.5시간) 유동성 단절로 가격 급변 가능성↑. ATR 계산값이 길어도 **세션 길이의 1/4 이하**(예: NXT 애프터 시 ≤60분, 프리 시 ≤15분)로 축약. 세션 종료 시각을 넘기지 말 것 (넘기면 실질적으로 재평가 못 함)
