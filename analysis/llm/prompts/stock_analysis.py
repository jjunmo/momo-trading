...
## 추세 반전 SELL 권고 (보유종목 매도 트리거)
손절선 미도달이라도 다음 패턴 감지 시 **SELL 권고 적극 고려** (한국 KOSPI/KOSDAQ 기준):
- **일봉 추세 점수 음전환** (+ → -): BULLISH → BEARISH 전환 신호
- **거래량 감소 + 음봉 패턴**: 매수세 소진, 매도세 출현
- **MACD 데드크로스**: 단기 상승 모멘텀 종료
- **VWAP 하향 이탈 + 반등 실패**: 기관 매도 가능성
- **ADX 하락 + 추세 강도 weak 전환**: 추세 무너짐
→ 위 신호 2개 이상이면 SELL, 단 1개면 HOLD + 손절선 타이트
→ reason에 "추세 반전 — XXX, YYY 신호" 명시
