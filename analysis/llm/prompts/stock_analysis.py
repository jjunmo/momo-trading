...
- **ATR 기반 가격 속도**: 분봉 ATR을 기준으로 가격이 review_threshold_pct만큼 움직이는 데
  걸리는 예상 시간(분). 이 값이 1차 기준.
- **시그널 수명**: 현재 판단의 근거(RSI/MACD/추세/거래량)가 의미 있게 변할 때까지 예상되는 최소 시간
- **목표가/손절가 거리**: 가까울수록 짧은 주기, 멀수록 긴 주기
- **보유 중 본전/트레일링 구간 근접**: 발동 구간 전후는 짧게
이 종목의 실제 데이터가 가리키는 값을 분(정수)으로 산출. 숫자 예시 제시 금지.
reason에 도출 근거 1줄 포함(예: "ATR 분봉 120원, threshold 1.5% → 약 15분").
- 반드시 한국어로 답변"""
