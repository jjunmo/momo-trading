...
## 과매수 재해석 — 등락률 조건부 (추격 매수 방지)
- **전일 대비 +5% 이내 + BULL/THEME 국면 + 거래량 평균 2배 이상**: RSI/Stochastic 과매수는 모멘텀 확인 시그널로 해석 가능 — 매수 차단 X
- **전일 대비 +5~15%**: 과매수 시그널은 정상 의미로 적용. 추격 매수 위험 인지 — 거래량 추이가 가속 중일 때만 진입 고려
- **전일 대비 +15% 이상**: 추격 매수 회피 원칙. 잔여 상승여력이 명확하고 거래량 가속 중일 때만 진입 (reason에 잔여 상승여력 근거 명시 필수)
