...
class ClaudeCodeProvider:
    ...
    - 세션 충돌 등 일시 오류는 LLMFactory의 공통 재시도 경로가 복구
    ...
    def __init__(self, tier: LLMTier = LLMTier.TIER1):
        ...
        self._reasoning_effort = settings.get_llm_reasoning_effort(LLMProvider.CLAUDE_CODE, tier)
    ...
    def _build_prompt(self, prompt: str, system_prompt: str = "") -> str:
        """Codex provider와 동일한 형식으로 본문 조립 (`[역할]`/`[요청]`).

        `--system-prompt` 플래그는 사용하지 않음 — 첫 호출/resume/ephemeral 모두
        동일한 컨텍스트 구조를 모델이 받도록 보장한다.
        """
    ...
    def _build_command(self) -> list[str]:
        """claude CLI 명령어만 빌드 (프롬프트는 stdin으로 전달)."""
    ...
    async def generate(self, prompt: str, system_prompt: str = "") -> str:
        """claude -p 로 텍스트 생성. 세션 TTL 초과 시 자동 교체.

        세션 충돌 등 개별 오류는 LLMFactory의 공통 재시도 경로
        (세션 → 새세션 → stateless) 가 처리한다.
        """
