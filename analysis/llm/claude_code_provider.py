...
class ClaudeCodeProvider:
    ...
    세션 안정성:
    - TTL 30분 초과 시 자동 새 세션 생성
    - "already in use" 충돌 시 새 세션으로 교체
    - reset_session()으로 에러 복구
    """
    ...
    _session_started_at: float = 0.0  # 세션 시작 시간
    SESSION_TTL_SECONDS: float = 1800.0  # 30분
    ...
    def pause_session(cls) -> str | None:
        ...
        cls._active_session_id = None
        # _session_initialized, _session_started_at은 보존 (resume 시 그대로 사용)
        return sid
    ...
    def reset_session(cls) -> None:
        ...
        cls._session_initialized = False
        cls._session_started_at = 0.0
        logger.debug("Claude Code 세션 상태 초기화 (에러 복구)")

    @classmethod
    def _is_session_expired(cls) -> bool:
        """세션 TTL 초과 여부 확인"""
        if not cls._active_session_id or cls._session_started_at == 0:
            return False
        return (time.time() - cls._session_started_at) > cls.SESSION_TTL_SECONDS

    @classmethod
    def _rotate_session(cls, reason: str = "") -> str:
        """현재 세션 폐기 후 새 세션 시작 (맥락은 잃지만 세션은 유지)"""
        old_id = cls._active_session_id
        old_short = old_id[:8] if old_id else "없음"
        new_id = cls.start_session()
        logger.info("세션 교체: {} → {} (사유: {})", old_short, new_id[:8], reason or "unknown")
        return new_id
    ...
    def _build_command(self, prompt: str, system_prompt: str = "") -> tuple[list[str], str]:
        """CLI 명령어 + 실제 프롬프트 빌드

        Returns: (cmd_list, actual_prompt)
        """
        ...
        return cmd, actual_prompt

    async def generate(self, prompt: str, system_prompt: str = "") -> str:
        """claude -p 로 텍스트 생성

        세션 TTL 초과 시 자동 교체.
        세션 충돌("already in use") 시 새 세션 생성.
        """
        # 세션 TTL 초과 시 자동 교체
        if self._active_session_id and self._is_session_expired():
            self._rotate_session(reason="TTL 만료 (30분)")

        # 세션 사용 시 직렬화 (같은 세션에 동시 resume 방지)
        if self._active_session_id:
            async with self._get_lock():
                # 락 안에서 커맨드 빌드 (세션 ID 일관성 보장)
                cmd, actual_prompt = self._build_command(prompt, system_prompt)
                try:
                    ...
                    if "already in use" in str(e):
                        logger.warning("세션 충돌 → 새 세션으로 교체: {}", str(e)[:80])
                        self._rotate_session(reason="already in use 충돌")
                        new_cmd, new_prompt = self._build_command(prompt, system_prompt)
                        return await self._execute(new_cmd, new_prompt)
                    raise
        else:
            cmd, actual_prompt = self._build_command(prompt, system_prompt)
